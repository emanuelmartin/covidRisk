...
class Laboratorio extends Component {
  ...
  onBillPress() {
    ...
    this.props.addBill({
      ...
      bill: { Type: 'laboratorio', laboratorio: this.state.Productos },
      total
    });
  }
    ...
    if (parseFloat(this.state.recibido) < (total.subtotal + total.iva).toFixed(2)) {
      ...
      this.props.payment(
        { pago: total,
          ...
          lista: { Type: 'laboratorio', farmacia: [], imagen: [], laboratorio: this.state.Productos, rehabilitacion: [], otros: [] },
  ...
  onCorteConfirm(retiro) {
    ...
    if (parseRetiro > 0 && parseRetiro <= this.props.Caja.efectivo) {
      ...
      this.props.corte('laboratorio', parseRetiro);
    } else {
      let error = '';
      if (parseRetiro <= 0) {
                ...
                if (text !== '') {
                  if (this.state.Productos.length === 0) {
                    ...
                    constrain: [{ type: 'startsWith', variable: 'nombre', text },
                      { type: 'containedIn', variable: 'tipo', text: ['laboratorio', 'imagen'], bool: 'and' }]
                    });
                  } else {
