...
class AdminLaboratorio extends Component {
  ...
  listaProductos() {
    ...
    if (this.state.Cuenta.objectId) {
            this.state.Productos = this.state.Cuenta.cuenta.laboratorio;
  ...
  confirmar() {
    if (this.state.Productos && this.state.Cuenta) {
      ...
      this.state.Productos.forEach((producto) => {
        if (!producto.integrado) pendienteLaboratorio = true;
      })

      console.log('Cuenta', Cuenta.estado)
      pedido.set('objectId', Cuenta.objectId);
      pedido.set('pendienteLaboratorio', pendienteLaboratorio);
...
export default connect(mapStateToProps, { queryFunc, cleanFunc, multiWrite, cleanVariable })(AdminLaboratorio);
