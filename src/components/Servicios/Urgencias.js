...
class Urgencias extends Component {
  ...
  onBillPress() {
    ...
    this.props.addBill({
      ...
      bill: { Type: 'urgencias', otros: this.state.Productos },
      total
    });
  }
    ...
    if (parseFloat(this.state.recibido) < (total.subtotal + total.iva).toFixed(2)) {
      ...
      this.props.payment(
        { pago: total,
          ...
          lista: { Type: 'urgencias', farmacia: [], imagen: [], laboratorio: [], rehabilitacion: [], otros: this.state.Productos },
  ...
  onCorteConfirm(retiro) {
    ...
    if (parseRetiro > 0 && parseRetiro <= this.props.Caja.efectivo) {
      ...
      this.props.corte('urgencias', parseRetiro);
    } else {
      let error = '';
      if (parseRetiro <= 0) {
