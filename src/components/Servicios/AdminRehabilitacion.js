...
class AdminRehabilitacion extends Component {
  ...
  componentDidMount() {
    this.props.queryFunc({
      type: 'equalTo',
      object: 'Cuenta',
      variable: 'pendienteRehabilitacion',
      text: true,
    include: ['autor'] })
  }
  ...
  listaProductos() {
    ...
    if (this.state.Cuenta.objectId) {
            this.state.Productos = this.state.Cuenta.cuenta.rehabilitacion;
  ...
  confirmar() {
    if (this.state.Productos && this.state.Cuenta) {
      ...
      let pendienteRehabilitacion = false;
      this.state.Productos.forEach((producto) => {
        if (!producto.integrado) pendienteRehabilitacion = true;
      })

      console.log('Cuenta', Cuenta.estado)
      pedido.set('objectId', Cuenta.objectId);
      pedido.set('pendienteRehabilitacion', pendienteRehabilitacion);
...
export default connect(mapStateToProps, { queryFunc, cleanFunc, multiWrite, cleanVariable })(AdminRehabilitacion);
